...
SlamToolbox::SlamToolbox(ros::NodeHandle &nh)
    : solver_loader_("slam_toolbox", "karto::ScanSolver"),
      processor_type_(PROCESS), first_measurement_(true), nh_(nh),
...
/*****************************************************************************/
{
  ...
  while (ros::ok()) {
    updateMap();
    // TODO: Crash occuring on graph visualization -- cause not verified at this
    // time if(!isPaused(VISUALIZING_GRAPH))
    // {
...
/*****************************************************************************/
{
  ...
  if (nh_.getParam("map_file_name", filename)) {
    ...
    if (nh_.getParam("map_directory", map_dir)) {
      filename.insert(0, map_dir);
    }
...
SlamToolbox::getLaser(const sensor_msgs::LaserScan::ConstPtr &scan)
/*****************************************************************************/
{
...
tf2::Stamped<tf2::Transform> SlamToolbox::setTransformFromPoses(
    const karto::Pose2 &corrected_pose, const karto::Pose2 &karto_pose,
    const ros::Time &t, const bool &update_reprocessing_transform)
/*****************************************************************************/
{
...
SlamToolbox::getLocalizedRangeScan(karto::LaserRangeFinder *laser,
...
/*****************************************************************************/
karto::LocalizedRangeScan *SlamToolbox::addScan(karto::LaserRangeFinder *laser,
...
/*****************************************************************************/
karto::LocalizedRangeScan *
SlamToolbox::addScan(karto::LaserRangeFinder *laser,
                     const sensor_msgs::LaserScan::ConstPtr &scan,
                     karto::Pose2 &karto_pose)
/*****************************************************************************/
{
  ...
  karto::LocalizedRangeScan *range_scan =
      getLocalizedRangeScan(laser, scan, karto_pose);
...
/*****************************************************************************/
{
  ...
  karto::LaserRangeFinder *laser =
      dynamic_cast<karto::LaserRangeFinder *>(dataset_->GetLasers()[0]);
...
{
  if (req.match_type ==
      slam_toolbox_msgs::DeserializePoseGraph::Request::UNSET) {
  ...
  case procType::START_AT_GIVEN_POSE:
    ...
    process_near_pose_ = std::make_unique<karto::Pose2>(
